    ...
    let result = new Promise((resolve, reject) => {
      ...
      .then(() => {
        ...
        let isPostValid = !!currentContent.match(
          /(http|https):\/\/(www\.steemit\.com\/|steemit\.com\/|busy\.org\/|www\.busy\.org\/|steempeak\.com\/|www\.steempeak\.com\/|partiko\.app\/|www\.partiko\.app\/|esteem\.app\/|www\.esteem\.app\/)/g
        );
        if (isPostValid) {
          return;
        } else {
        ...
        return checkLastPost(currentUserId)
          .then(data => {
            if (!!data) {
              ...
              // First Time
              msg.reply(
                `Welcome, seems like it is the first time you post here.`
              );
            }
            // UPDATE TIME QUERY
      ...
      .catch(err => {
        ...
        switch (err) {
          case "NOT_REGISTERED":
            msg.reply(
              "'you are not yet registered. Go to #PP-registration channel and register your steem username."
            );
            break;
          case "NOT_VALID_LINK":
            msg.reply(
              "You did not include link to steemit.com / busy.org / steempeak.com / partiko.app / esteem.app"
            );
            break;
          case "NOT_YET_TIME":
            msg.reply(
