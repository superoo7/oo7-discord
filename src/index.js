...
// Controller
import { checkRegisteredUser, checkLastPost, updateTime } from './controller/user';
...
client.on('message', msg => {
  ...
  if (currentUserId === config.botId) {
    ...
    let currentCreatedTime = getDateTimeFromTimestamp(currentCreatedTimestamp);
    ...
    let result = new Promise((resolve, reject) => {
      ...
      .then(() => {
        ...
        let isPostValid = !!currentContent.match(/(http|https):\/\/(www\.steemit\.com\/|steemit\.com\/)/g);
        if (isPostValid) {
          return;
        } else {
        ...
        return checkLastPost(currentUserId)
          .then(data => {
            if (!!data) {
              ...
              // First Time
              msg.reply(`seems like it is the first time you post here.`);
            }

            // Quality Check
            // ============================================================
            // Parse link (asd https://steemit.com/ asd => https://steemit.com)
            let link = currentContent.match(/(https?:\/\/[^\s]+)/g);
            ...
            if (link.length === 1) {
              ...
              if (authorName.charAt(0) === '@' && !!permlinkName) {
                return main(authorName.substr(1), permlinkName, postConfig).then(postData => {
                    if (postData.msg === 'CHEETAH' || postData.msg === 'OLD_POST' || postData.msg == 'POST_NOT_FOUND' ) {
                        throw postData.msg;
                    }
                    return postData;
                }).catch(err => {throw err})
          ...
          .then(postData => {
            return upvote(process.env.STEEM_POSTING, process.env.STEEM_USERNAME, postData.author, postData.permlink, postData.weightage)
                            .then(upvoteData => {
                                if (upvoteData === 'ERROR') {
                                    throw 'NO_UPVOTE';
                                  } else {
                                    msg.reply(postData.msg);
                                    Promise.resolve();
                                  }
                            }).catch(err => {throw err})
          })
      ...
      .catch(err => {
        ...
        switch (err) {
          case 'NOT_REGISTERED':
            ...
            break;
          case 'NOT_VALID_LINK':
            msg.reply('You did not include link to steemit.com');
            break;
          case 'NOT_YET_TIME':
            msg.reply(`Please wait for ${timeConvertMessage(convert(config.timeAllowed - timeDiff))}`);
            break;
          case 'CHEETAH':
            msg.reply('Your post is voted by @cheetah');
            break;
          case 'OLD_POST':
          ...
          case 'POST_NOT_FOUND':
            ...
            break;
          case 'NO_UPVOTE':
            msg.reply('Can\'t upvote, it could be upvoted already');
            break;
          default:
            msg.reply('ERROR');
            break;
